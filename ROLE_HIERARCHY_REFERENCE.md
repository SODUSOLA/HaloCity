...
No `WHERE` filter on anything. Every incident, every zone, every marshal, every maintenance ticket, every escalation rule, city-wide, with no scoping at all.

### Permissions by Module

| Module | Capability |
|---|---|
| Incidents | View all, update any status transition (not just the sequential ones marshals are limited to), assign a marshal to any incident |
| Zones | Full CRUD — create, edit, activate/deactivate |
| Marshals | Assign marshals to zones, assign marshals to specific incidents, dispatch corridor alerts to one or more zones |
...
| Dashboard | Only role with access to all 6 aggregation endpoints (summary, live feed, marshal map, zone density, infrastructure status, pending escalations) |

### Real-Time Channel

Joins the `admin` room only — but that one room receives everything: every `incident:created`, every `incident:updated`, every `incident:escalated`, marshal location pings, corridor dispatch confirmations.

**Receives:** Tier-2 escalation notices — the backstop. If a marshal doesn't acknowledge an incident in time, it doesn't just sit there; it routes specifically to admin.
...
Self-registerable (role selector: Citizen or Marshal) or admin-assigned.

**JWT payload:** `{ id, role: MAYOR, zoneId }` — zoneId populates once admin assigns them to a zone; before that, it's effectively unset.

### Data Visibility

Zone-scoped: `WHERE zoneId = user.zoneId`. A marshal sees every incident in their assigned zone, not just ones personally assigned to them — zone visibility and personal assignment are two separate, stacked layers of access.

### Permissions by Module

| Module | Capability |
|---|---|
| Incidents | View zone incidents, move status forward (Acknowledge → In Progress → Resolved) — cannot assign marshals, cannot view incidents in other zones |
...
| Dashboard | No access — marshals have no aggregate/city-wide view at all |

### Real-Time Channels

Two, simultaneously: `mayor:{mayorId}` (personal — assignment notices, direct instructions) and `zone:{zoneId}` (shared with every other marshal in that zone — new incidents, corridor alerts).
...
Cannot see incidents outside their zone, cannot see the maintenance ticket list (only individual asset status), cannot see any dashboard aggregate, cannot manage other users, cannot assign other marshals, cannot skip status transitions (the backend enforces the sequence — a marshal can't jump straight from Pending to Resolved).
...
Strictly their own: `WHERE reporterId = user.id`. Zero visibility into anyone else's reports, any zone-wide data, or any marshal/admin activity beyond what's reflected back on their own report's status.
...
| Everything else | No access — no marshals module, no maintenance, no escalation rules, no dashboard |
...
Authority strictly narrows as you go down — admin sees everything, marshal sees a slice, citizen sees only their own slice of that slice.
...
- **Marshals** are the only ones who continuously report live position
